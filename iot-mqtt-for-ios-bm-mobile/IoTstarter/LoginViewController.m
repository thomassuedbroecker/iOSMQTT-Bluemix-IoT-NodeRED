 ...
 *************************************************************************/
 ...
 - (void)viewWillAppear:(BOOL)animated
 {
     ...
     self.organizationField.placeholder = @"gq0561";
     // IOTOrgPlaceholder;
     self.deviceIDField.placeholder = @"iPhone6s-123456789";
     // IOTDevicePlaceholder;
     self.authTokenField.placeholder = @"MyAuthToIoT2016";
     // IOTAuthPlaceholder;

     if (![appDelegate.organization isEqualToString:@""])
     {
         self.organizationField.text = @"gq0561";
         appDelegate.organization;
     }
     if (![appDelegate.deviceID isEqualToString:@""])
     {
         self.deviceIDField.text = @"iPhone6s-123456789";
         appDelegate.deviceID;
     }
     if (![appDelegate.authToken isEqualToString:@""])
     {
         self.authTokenField.text = @"MyAuthToIoT2016";
 ...
 - (void)viewDidLoad
 {
     ...
     [IMFLogger setLogLevel:IMFLogLevelLog]; // setting the verbosity filter. Change to Info to get more logging output
     ...
     self.logger = [IMFLogger loggerForName:@"IoTMobileClient"];
     [_logger logDebugWithMessages:@"initializing bluemix client..."];
     ...
     [request sendWithCompletionHandler:^(IMFResponse *response, NSError *error) {
         if (error){
             NSLog(@"BLUEMIX - Error :: %@", [error description]);
         } else {
             NSLog(@"BLUEMIX - Response :: %@", [response responseText]);
         }
     }];
     // ****************************************
 }
